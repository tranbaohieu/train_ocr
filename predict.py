...
def main():
    ...
    config['weights'] = './weights_kalapa/vgg_seq2seq_score_0.9524.pth'
    detector = Predictor(config)
    # img = Image.open("/media/hwr/TK/OCR/public_test/images/215/17.jpg")
    # s = detector.predict(img)[0]
    # print(s)
    paths = glob.glob("./OCR/public_test/images/*/*")
    res = {"id" : [], "answer" : []}
    for path in tqdm.tqdm(paths):
        ...
        res["answer"].append(s)
    df = pd.DataFrame(res)
    df.to_csv(os.path.join("./OCR/public_test", "sample_submission.csv"), index=False)
